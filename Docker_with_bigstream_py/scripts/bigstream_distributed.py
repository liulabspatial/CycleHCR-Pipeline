...
def main():
    ...
    # distributed parameter (optional params) #
    parser.add_argument('--distribute',dest='distribute',type=str,help='0 is in_memory, 1 is distributed',required=False,metavar='',default='0')
    parser.add_argument('--blocksize',dest='blocksize',type=str,help='distributed blocksize',required=False,metavar='',default='128,256,256')
    parser.add_argument('--n_workers',dest='n_workers',type=str,help='n_workers passed to bigstream cluster_kwarg',required=False,metavar='',default='4')
    parser.add_argument('--threads_per_worker',dest='threads_per_worker',type=str,help='threads_per_worker passed to bigstream cluster_kwarg',required=False,metavar='',default='10')
    parser.add_argument('--memory_limit',dest='memory_limit',type=str,help='memory_limit passed to bigstream cluster_kwarg',required=False,metavar='',default='60GB')
    parser.add_argument('--outzarr',dest='outzarr',type=str,help='output registered image as zarr format in addition to tiff',required=False,metavar='',default='0')
    ...
    ### parameters format for distributed ###
    distribute = int(args.distribute) # will do 0,1 for distributed
    blocksize = [int(x) for x in args.blocksize.split(',')] # blocksize is a list of integers
    n_workers = int(args.n_workers) # n_workers is an integer
    threads_per_worker = int(args.threads_per_worker) # threads_per_worker is an integer
    memory_limit = str(args.memory_limit) # memory limit is a string
    outzarr = int(args.outzarr) # outformat as tiff or zarr
    ...
    cluster_kwargs = {
        'n_workers':n_workers, # I have 24 cores, 48 logical processors
        'threads_per_worker':threads_per_worker, # per worker, 4x10 threads, leaves me with 4 cores and 8 threads
        'memory_limit':memory_limit, # per worker?, 240 memory total, which is less than 255GB memory available on my pc
    }    
    ...
    affine_deform_zarr = os.path.join(outdir_transform,transform_prefix+".zarr")
    ...
    if os.path.exists(transform_checkpoint_path) and not args.force:
        ...
        ### if there are missing channels, perform registration using the existing transform data ###
            ...
            if os.path.exists(affine_deform_tiff):
            ...
            elif os.path.exists(affine_deform_zarr):
                affine_deform = zarr.open(affine_deform_zarr, mode='r')
            ...
            for c in channels_missing:
                ...
                if distribute == 0:
                    ...
                    print('apply_transform in memory')
                    reg = apply_transform(
                    ...
                    print('distributed_apply_transform')
                    # prefix for the registered image #
                    reg_prefix = 'reg_'+movBT[0]+'_'+movBT[1]+'_'+c+'_'+args.res
                    # save as zarr if outzarr = 1 #
                    if outzarr == 0:
                        write_path = None
                    elif outzarr == 1:
                        reg_zarr = os.path.join(outdir_registered,reg_prefix+'.zarr')
                        write_path = reg_zarr
                    reg = distributed_apply_transform(
                        fix_zarr=fix,
                        mov_zarr=mov,
                        fix_spacing=fix_spacing,
                        mov_spacing=mov_spacing,
                    ...
                    print("Invalid value for --distribute argument.")
    ...
    ### if transformation matrix does not exist, then perform transform followed by registration ###
        ...
        if distribute == 0:
            ...
            print('alignment_pipeline in memory')
            ...
            print('distributed_piecewise_alignment_pipeline')
            affine_deform = distributed_piecewise_alignment_pipeline(
                fix=fix,
                mov=mov,
                fix_spacing=fix_spacing,
                mov_spacing=mov_spacing,
